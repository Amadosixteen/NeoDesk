...
// remove just one app from all the permissions: tccutil reset All com.geatel.rustdesk
...
fn correct_app_name(s: &str) -> String {
    ...
    if let Some(bundleid) = get_bundle_id() {
        s = s.replace("com.geatel.rustdesk", &bundleid);
    }
...
pub fn write_plists() -> ResultType<()> {
    let daemon_plist_path = format!(
        "/Library/LaunchDaemons/com.geatel.{}_service.plist",
        crate::get_app_name()
    );
    let agent_plist_path = format!(
        "/Library/LaunchAgents/com.geatel.{}_server.plist",
        crate::get_app_name()
    );
    let Some(daemon_plist) = PRIVILEGES_SCRIPTS_DIR.get_file("daemon.plist") else {
...
pub fn update_from_dmg_as_root(dmg_path: &str, expected_version: &str) -> ResultType<()> {
    ...
    let agent_plist = format!("/Library/LaunchAgents/com.geatel.{}_server.plist", app_name);
    let daemon_plist = format!("/Library/LaunchDaemons/com.geatel.{}_service.plist", app_name);
    ...
    let daemon_label = format!("com.geatel.{}_service", app_name);
    let agent_label = format!("com.geatel.{}_server", app_name);
