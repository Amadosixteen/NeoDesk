...
/// The Linux bundle keeps the core library at lib/libneodesk.so next to the
...
DynamicLibrary _openLinuxCoreLib() {
  final bundled =
      '${File(Platform.resolvedExecutable).parent.path}/lib/libneodesk.so';
  try {
  ...
  return DynamicLibrary.open('libneodesk.so');
}
...
class PlatformFFI {
  ...
  NeodeskImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libneodesk.so')
        : isLinux
            ? _openLinuxCoreLib()
            : isWindows
                ? DynamicLibrary.open('libneodesk.dll')
                :
  ...
  void _startListenEvent(NeodeskImpl neodeskImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = neodeskImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
